...
<style>
    ...
    .metric-card {
        background: #111622; border: 1px solid #1e2638;
        border-radius: 12px; padding: 12px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.25); margin-bottom: 8px;
    }
    ...
    .top-status-bar {
        background: #111622; border: 1px solid #1e2638;
        border-radius: 10px; padding: 10px 16px; margin-bottom: 15px; font-weight: 600; font-size: 13px; line-height: 1.5;
    }
</style>
""",
...
exchange_name = st.sidebar.selectbox("Select Exchange", ["mexc", "binance", "bybit"], index=0)
...
st.markdown(f"## ⚡ Research Lab — Multi-Asset Signal & Trade Engine")
...
if not df.empty and len(df) >= 3 and len(bids) > 0 and len(asks) > 0:
  ...
  def compute_signal(df_in, bids_in, asks_in, history):
    ...
    try:
      ...
      paper_results = {
          "OFI": -0.204, "TSMOM": 0.850, "MICRO": -0.050,
          "AVST": 0.120, "INVAR": 0.450, "VPIN": -0.310,
          "LAMBDA": 0.080, "PIN": -0.150, "LOB_IMB": -0.220, "FLOW_IMB": 0.300,
      }
      final_score = -0.136
      evolved_weights = {k: 0.10 for k in paper_results.keys()}
    ...
    trajectory_dir = (
        "LONG" if final_score >= 0.15 else ("SHORT" if final_score <= -0.15 else "NEUTRAL")
    )
  ...
  dir_color = (
      "#00e676" if signal["direction"] == "LONG" else ("#ff5252" if signal["direction"] == "SHORT" else "#38bdf8")
  )
  ...
  with m1:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">🟠 {selected_symbol}</div>'
        f'<div class="metric-value-green">${close_val:,.2f}</div>'
        f'<div style="font-size:11px; color:#00e676;">+{pct_change:.2f}% (24h)</div></div>',
        unsafe_allow_html=True,
    )
  with m2:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Net Score</div>'
        f'<div class="metric-value-red">{signal["score"]:+.3f}</div></div>',
        unsafe_allow_html=True,
    )
  with m3:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Signal</div>'
        f'<div style="font-size:16px; font-weight:700; color:{signal_card_color}; margin-top:4px;">{signal["direction"]}</div></div>',
        unsafe_allow_html=True,
    )
  with m4:
    beam_val_str = f"${signal['beam']:,.2f}"
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Target (BEAM)</div>'
        f'<div class="metric-value-blue">{beam_val_str}</div></div>',
        unsafe_allow_html=True,
    )
  with m5:
    fig_gauge = go.Figure(
  ...
  with m6:
    st.markdown(
        f'<div class="metric-card"><div class="metric-label">Reset In</div>'
        f'<div style="font-size:16px; font-weight:700; color:#ffffff; margin-top:4px;">{mins_rem}m {secs_rem}s</div></div>',
        unsafe_allow_html=True,
    )
  ...
  auto_trade_mode = st.checkbox("🤖 Enable Fully Automatic Trading on Signal", value=False)
  ...
  with t_col1:
    trade_action = st.selectbox(
        "Action",
        (["BUY / LONG", "SELL / SHORT"] if signal.get("direction") != "SHORT" else ["SELL / SHORT", "BUY / LONG"]),
    )
  with t_col2:
    order_size_usdt = st.number_input("Size (USDT)", min_value=10.0, max_value=10000.0, value=100.0, step=10.0)
  with t_col3:
    leverage_val = st.slider("Leverage (x)", 1, 50, 10)
  with t_col4:
    sl_pct = st.number_input("Stop Loss %", min_value=0.1, max_value=10.0, value=1.5, step=0.1)
    tp_pct = st.number_input("Take Profit %", min_value=0.1, max_value=50.0, value=3.0, step=0.1)
  with t_col5:
    st.markdown("<br>", unsafe_allow_html=True)
    execute_btn = st.button("⚡ Execute Trade Order", use_container_width=True, type="primary")

  if execute_btn:
    active_exchange = get_exchange_connection(exchange_name, api_key, api_secret, trading_mode)

    if auto_trade_mode:
      direction_tag = signal["direction"] if signal.get("direction") else "LONG"
    ...
    try:
      if trading_mode == "Live/Real" and active_exchange:
        ...
        order_res = active_exchange.create_market_order(selected_symbol, side_str, qty)
        st.success(f"Live Order Successful on {exchange_name.upper()}! ID: {order_res.get('id', 'N/A')}")
      else:
        st.info(f"Paper Trade Simulated on {exchange_name.upper()}! Dir: {direction_tag} | Entry: ${entry_p:,.2f}")
    except Exception as e:
      st.error(f"Trade Execution Failed on {exchange_name.upper()}: {str(e)}")
  ...
  with col_chart:
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#111622", plot_bgcolor="#111622", margin=dict(l=5, r=5, t=5, b=5),
    )
  ...
  with col_side:
    ...
    st.markdown(
        '<div class="metric-card"><div style="display:flex; justify-content:space-between; margin-bottom:6px;"><span>Market Cap</span> <b>$2.28T <span style="color:#00e676;">+1.25%</span></b></div>'
        ...
        '<div style="display:flex; justify-content:space-between;"><span>Funding Rate</span> <b>0.0102%</b></div></div>',
        unsafe_allow_html=True,
    )
  ...
  with b3:
    ...
    st.markdown(
        '<div class="metric-card" style="height:240px;"><div style="display:flex; justify-content:space-between; padding:4px 0;"><span>OBI (Weighted)</span> <b style="color:#ff5252;">-0.154</b></div>'
        ...
        '<div style="display:flex; justify-content:space-between; padding:4px 0;"><span>Liquidity Score</span> <b style="color:#f59e0b;">58 / 100</b></div></div>',
        unsafe_allow_html=True,
    )
  ...
  if st.session_state.trade_history_log:
    ...
    with tab_d:
      ...
      with w4:
        ...
        st.markdown(f'<div class="metric-card"><div class="metric-label">Neutral</div><div style="font-size:18px; font-weight:700; color:#38bdf8; margin-top:4px;">{neu_d}</div></div>', unsafe_allow_html=True)

    with tab_w:
      ww1, ww2, ww3, ww4 = st.columns(4)
      with ww1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Total Signals (Weekly)</div><div class="metric-value-blue">{tot_w}</div></div>', unsafe_allow_html=True)
      with ww2:
      ...
      with ww3:
        ...
        st.markdown(f'<div class="metric-card"><div class="metric-label">Avg Score (Weekly)</div><div style="font-size:18px; font-weight:700; color:{sc_col_w}; margin-top:4px;">{avg_s_w:+.3f}</div></div>', unsafe_allow_html=True)
      with ww4:
        neu_w = tot_w - (long_w + short_w)
        st.markdown(f'<div class="metric-card"><div class="metric-label">Neutral</div><div style="font-size:18px; font-weight:700; color:#38bdf8; margin-top:4px;">{neu_w}</div></div>', unsafe_allow_html=True)

    with tab_m:
      wm1, wm2, wm3, wm4 = st.columns(4)
      with wm1:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Total Signals (Monthly)</div><div class="metric-value-blue">{tot_m}</div></div>', unsafe_allow_html=True)
      with wm2:
      ...
      with wm3:
        ...
        st.markdown(f'<div class="metric-card"><div class="metric-label">Avg Score (Monthly)</div><div style="font-size:18px; font-weight:700; color:{sc_col_m}; margin-top:4px;">{avg_s_m:+.3f}</div></div>', unsafe_allow_html=True)
      with wm4:
        neu_m = tot_m - (long_m + short_m)
        st.markdown(f'<div class="metric-card"><div class="metric-label">Neutral</div><div style="font-size:18px; font-weight:700; color:#38bdf8; margin-top:4px;">{neu_m}</div></div>', unsafe_allow_html=True)
