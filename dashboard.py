...
st.sidebar.markdown("---")
st.sidebar.subheader("🔄 Built-in Auto-Refresh")
...
bids_mock = np.array([[base_p - spread, np.random.uniform(2, 10)], [base_p - spread*2, np.random.uniform(5, 20)]])
asks_mock = np.array([[base_p + spread, np.random.uniform(2, 10)], [base_p + spread*2, np.random.uniform(5, 20)]])
...
risk_metrics = risk_engine.calculate_risk_metrics(
    liquidation_volumes=np.array([2500, 8900, 1200]),
