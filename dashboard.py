...
if not df.empty and len(df) >= 3 and len(bids) > 0 and len(asks) > 0:
    ...
    def compute_signal(df_in, bids_in, asks_in, history):
        ...
        # Updated direction mapping: Long for upside (green), Short for downside (red)
        trajectory_dir = "Long" if final_score >= 0.15 else ("Short" if final_score <= -0.15 else "Sideways")
    ...
    signal_color = "#00e676" if signal['direction'] == "Long" else ("#ff5252" if signal['direction'] == "Short" else "#f59e0b")

    st.markdown(f"""
    <div class="top-status-bar">
        🔵 <b>[{selected_symbol}]</b> | Timeframe: {selected_tf_label} | <b>SIGNAL:</b> <span style="color:{signal_color};">{signal['direction']}</span> &nbsp;|&nbsp; 
        Net Score: <span style="color:{signal_color};">{signal['score']:+.3f}</span> &nbsp;|&nbsp; Target (BEAM): <span style="color:#38bdf8;">${signal['beam']:,.2f}</span> &nbsp;|&nbsp; 
    ...
    with m2:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Net Score</div><div style="font-size:20px; font-weight:700; color:{signal_color};">{signal["score"]:+.3f}</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f'<div class="metric-card"><div class="metric-label">Signal</div><div style="font-size:14px; font-weight:700; color:{signal_color}; margin-top:4px;">{signal["direction"]}</div></div>', unsafe_allow_html=True)
    with m4:
    ...
    with col_chart:
        ...
        traj_color = "#00e676" if signal["direction"] == "Long" else ("#ff5252" if signal["direction"] == "Short" else "#f59e0b")
...
time.sleep(10)
st.run() if hasattr(st, "run") else None
